...
\usepackage{subcaption} % For multiple images stuck together.

% Tables.
\usepackage{booktabs} % For big tables.
\usepackage[table]{xcolor} % For colour coding in tables.
\usepackage{rotating} % Sideways tables.
\usepackage{adjustbox} % For big tables.

% Maths.
\usepackage{amsmath} % For aligning eqns.
\usepackage{siunitx} % For standard form numbers.
\newcommand{\RR}{\ensuremath{R^2}}
...
\title{Emulation of the United Kingdom Chemistry and Aerosols (version 13.9) Photolysis Schemes (Fast-JX version 6.4) with a Random Forest}
...
Atmospheric photolysis, the absoprtion and subsequent dissociation of molecules by sun light, directly affects air pollution and significantly influences the mechanisms of climate change.
% What's UKCA and UM
The Unified Model (UM) is the atmospheric science component of the United Kingdom's Earth System Model (UKESM), a fully coupled Earth system model. The United Kingdom Chemistry and Aerosols (UKCA) model is the atmospheric chemistry and aerosol component of the UM and performs the simulation of photolysis.

% Problem.
Among the various components of UKCA, photolysis is known to be computationally demanding. UKCA uses a combination of offline lookup tables and interactive calculations using Fast-Jx (v6.4) to simulate photolysis. This involves numerous calculations that encompass different chemical species across space and time, taking up approximately 20\% of UKCA's run-time, and approximately 25\% of UKCA's time spent on scientific calculations. Enhancing the efficiency of photolysis simulation could lead to faster execution of the entire Earth system model and reduce power consumption. 
% Aims.
...
% What was done.
UKCA photolysis rate coefficients for multiple reactions were emulated using a random forest, which was trained offline in python using physical input data from the UM, including solar zenith angle, temperature and cloud information, as well as shortwave solar fluxes from the radiation scheme. The random forest was then integrated into UKCA in Fortran and run online in the UM. 

% Accuracy.
The random forest was able to emulate UKCA photolysis rate coefficients accurately, with a coefficient of determination of 0.97 compared to the original scheme over all emulated reactions, and 0.76 to over 0.99 for individual reactions. When coupled to the UM, the results compared well between simulations with the standard interactive photolysis scheme and the random forest emulator. Ozone mixing ratios were... {summarise the impacts on concentrations of species} 
% Speedup.
...
% Atmospheric photolysis.
Atmospheric photolysis refers to the phenomenon in which molecules in the atmosphere are dissociated by solar radiation. As a result of some common photolysis processes, radicals can form, which are very reactive species, and they rapidly initiate subsequent atmospheric reactions \citep{finlayson-pitts_chemistry_2000}. This has considerable implications for air pollution. Atmospheric photolysis reactions affect phenomena such as smog in urban areas and the thinning of the stratospheric ozone layer, which seriously impact the health of people and the environment \citep{seinfeld_atmospheric_2016}. Including photolysis in climate models is necessary for obtaining accurate predictions of changes in atmospheric composition. The rates of photolysis reactions are influenced by various factors, including clouds, aerosols, atmospheric pressure and the time of day and year \citep{liang_chemical_2013}. All of these factors must be carefully taken into account when developing a comprehensive model that simulates this process.

% How photolysis is calculated.
The calculation of photolysis rates involves determining the intensity of solar radiation at different wavelengths, quantifying the absorption cross-sections and quantum yields for the reactions of the molecules, and accounting for factors such as the impact of atmospheric composition on the transfer of radiation, as well as altitude, latitude, time of day and season. The rate coefficient of the photolysis reaction is termed $J$ and has units of $s^{-1}$.  The J-value at a point in altitude, $z$, of a particular photolabile chemical, $i$, which absorbs light in a specific range of wavelengths, can be calculated as
\begin{equation}
J_i,z = \int_{0}^{\infty} \Phi_i (\lambda, T) \sigma_i (\lambda, T) F_z (\theta, \lambda) d \lambda 
\end{equation}
where $\Phi$ is the quantum yield for photolysis of the chemical, $\lambda$ is the wavelength, $T$ is the temperature, $\sigma$ is the absorption cross-section, $F$ is the flux of light, also known as the actinic flux, $\theta$ is the solar zenith angle and $d \lambda$ is the change in wavelength \citep{king_calculating_2013}. Flux refers to the difference in the amount of radiation reaching the molecule over time, which varies significantly at a point in space over the course of a day.
At a point in space and time in the atmosphere, the solar zenith angle (SZA, $\theta$) is the angle between the direction of incoming solar radiation and the local vertical line perpendicular to the Earth's surface at that point \citep{finlayson-pitts_chemistry_2000}. 

The photon flux, F, is calculated by {add some details here about how F is calculated -- this is a big part of what FastJx is doing so needs to come here so the reader understands why this is complex} 
J-values can range from around $10^{-16} s^{-1}$ to over $10^{-2} s^{-1}$ for different species in the atmosphere and vary greatly with altitude.

% UKESM, UM and UKCA.
The United Kindgom Earth System Model (UKESM) is a collaborative initiative involving the Met Office, the National Centre for Atmospheric Science (NCAS) and the UK academic community \citep{sellar_ukesm1_2019}. UKESM plays an important role in assessing climate forcing, projecting changes in air quality and quantifying the impacts of mitigation measures. It contributes to the Coupled Model Intercomparison Project (CMIP) \citep{eyring_overview_2016}. The atmospheric component of UKESM is the Unified Model (UM), developed by the Met Office, while the United Kingdom Chemistry and Aerosols (UKCA) model \citep{archibald_description_2020} serves as its atmospheric chemistry and aerosol component. The UM and UKCA code is extensive and complicated due to a variety of chemistry schemes and photolysis approaches, and there are various UM versions and many configurations that run on different computing infrastructures.

% Photolysis schemes in UKCA and how they work.
Different schemes are present in UKCA for photolysis calculations, including a legacy `2D' scheme based on the interpolation of tabulated values pre-calculated in the Cambridge 2D model \citep{abraham_united_2014}, and the Fast-JX scheme \citep{telford_implementation_2013}, suitable for tropospheric and stratospheric conditions. The primary photolysis scheme currently used in UKCA is Fast-JX, which is a more advanced and precise approach than the 2D scheme. % Strat scheme.
Fast-JX is intended to be used at atmospheric pressures greater than 20 Pa \citep{wild_fast-j_2000}. When using Fast-JX in the UM for configurations that include altitudes at which atmospheric pressure drops below 20 Pa, there are three options for how to handle the portion of lower pressure; Fast-JX may be used, photolysis may be approximated using the 2D lookup table of stratospheric rates, or both Fast-JX and the lookup table may be used and their results aggregated.
The most accurate of the options at pressures less than 20 Pa is considered to be the combination of both Fast-JX and the lookup table scheme. Hence, an emulation of UKCA photolysis should, ideally, not only emulate Fast-JX, but also emulate the combined results of Fast-JX and the lookup table in the upper stratosphere.

\begin{figure}
    \centering \includegraphics[width=\textwidth]{images/flame.png}
    \caption{\textit{Flame graph showing the timed processes during a run of the UM, from \cite{dichev_report_2021}. UKCA and photolysis are highlighted in green and blue, respectively, with their total percentages of the UM's run-time.}}
    \label{flamegraph}
\end{figure}

% Timings.
Among the various components of UKCA, photolysis is known to be computationally demanding. It involves numerous calculations that encompass different chemicals, locations, altitudes and time steps, making up approximately 20\% of UKCA's run-time, and about 25\% of UKCA's time spent on scientific calculations \citep{dichev_report_2021}. In each grid box, there is incoming and outgoing light, which can occur either directly or through scattering, involving neighbouring grid boxes. These effects can propagate across multiple boxes of the grid. The photolysis calculating routines are invoked for each grid box at hourly UKCA time steps. The flame graph in Figure \ref{flamegraph} shows an example of the timed processes during a run of the UM, of which photolysis accounts for 5.13\% of the total run-time \citep{dichev_report_2021}. These vary depending on the chemistry scheme chosen, the type of suite run and other options selected for the run.  

% Power usage, electricity bill cost and GWP.
Reducing energy usage is a priority for climate modellers and high-performance computing (HPC) centres. Running models such as UKESM in typical scientific workloads, comprising many multi-decadal simulations, requires megawatts of energy, incurring a global warming potential of tonnes of CO$_2$ equivalent greenhouse gas emissions (CO$_2$e) and financial costs of tens of thousands of GBP which often belong to the taxpayers \citep{jackson_emissions_2023}. Improving the computational efficiency of processes within the model, including photolysis, is essential for reducing its energy consumption.
...
A decision tree is a model containing many nodes, which iteratively compares the value of an input feature for each sample (define what a sample is) to a threshold value at every node and traverses a path through the tree depending on whether the input feature value exceeds the threshold value at each node, ending at a leaf node which contains a prediction value for the output feature \citep{tibshirani_elements_2001}. Each node considers one randomly chosen input feature.  Top-down, greedy binary decision trees were used, in which each parent node has two child nodes, with an input feature value less than or equal to the node's threshold leading to the left child and a value higher than the threshold leading to the right child. Figure \ref{tree diagram} represents a simplified example of this process. The tree shown in the Figure is a single-output model but a decision tree can also output a vector of prediction values at leaf nodes for a set of output features.

% Show maths eqns.
Chosen splits minimise the loss function at each node, $m$. 
% MSE.
With the mean squared error (MSE) loss function, the algorithm at each node minimises the equation,
\begin{equation}
...
A random forest is an ensemble of decision trees, in which each tree is trained on a subsample of the data and input features, and the output is the mean of the outputs from every tree \citep{james_introduction_2023}.
...
This project aims to determine whether a random forest can adequately emulate an atmospheric photolysis model. Ultimately, the objective is to use a random forest to accelerate UKCA photolysis without compromising its accuracy. 

Assuming a simulation throughput of approximately four model years per real day, even a 1\% improvement in overall UM performance would be significant, potentially enabling an additional two weeks of simulated time per real day and considerably reducing cumulative energy and financial costs. Accordingly, this project aims for an overall UM speedup of 1\%, corresponding to approximately a 4\% speedup of UKCA and approximately a 20\% speedup of photolysis. The target predictive skill of the emulator is to achieve a coefficient of determination of at least 0.9 for every reaction, in line with high-performing models in recent literature, such as \cite{liu_deep_2025} and \cite{pan_development_2025}, ideally throughout the spatial and temporal range of the test data. A coefficient of determination of 0.9 qualitatively puts the emulated J-rates within both measurement and computational calculation uncertainty (i.e. we don't feel there is evidence that we know the magnitude of J-rates across the atmosphere to higher precision). This aim is ambitious because the model must emulate photolysis throughout the whole global grid up to a height of 85 km and in all atmospheric conditions, which is a more complex task than those described in \cite{liu_deep_2025} and \cite{pan_development_2025}. 
A list of emulated photolysis reactions is provided in Appendix \ref{rxns} and an explanation of the performance metrics used is provided in Appendix \ref{metrics}.\\

The specific research aims are:\\

1. Develop an ML model to accurately emulate UKCA photolysis rate coefficients, with a global coefficient of determination of at least 0.9 for each emulated reaction.\\

2. Use the emulator to speed up the run time of UKCA within the UM, reducing the run time of the photolysis portion of UKCA by at least 20\%.
...
% Dataset.
A dataset was compiled from UM outputs, comprising J-values calculated by the UKCA photolysis scheme and physical fields related to photolysis, including shortwave fluxes, SZA, temperature, pressure, humidity and cloud fraction. 
% Physical data.
The physical data were used as inputs to the random forest, while the J-values were used as output targets.
% 1 year.
A year of data was output from the UM at a daily resolution of 24 hours, 85 vertical levels, 144 latitudes and 192 longitudes.
...
% Flattening.
The Scikit-learn Python package was used for development and testing of different ML methods because it provides a vast library of pre-built models of various ML algorithms which can easily be adjusted in Python \citep{pedregosa_scikit-learn_2011}. Other packages tend to be specifically designed for one approach and lack the range of tools and generalisability available from Scikit-learn. For most applications, Scikit-learn requires input data to be structured as a two-dimensional array, in which one dimension is samples and the other is features. The output data from the UM were five-dimensional, with dimensions of time, altitude, latitude, longitude and field. `Field' is the name given to output items, such as each J-value and physical parameters from the UM. The data were flattened from five dimensions to two without losing the spatial or temporal information by including all fields, as well as time, altitude, latitude and longitude as features in the resulting array.

% Raw data size.
The resulting dataset contained approximately 56 million data points in up to 18 GB of single-precision data for each simulated day. A year of output data occupied up to 6.5 TB of storage. It was then necessary to sample the one-year dataset to reduce its size.
% Conclusion.
...
% Day-file sampling.
According to the final design, each day of data in one year was sampled individually, from uniform random indices, to form an appropriately-sized subsample of 500,000 daily data points. The subsamples from each day were then joined to form one file containing a single-precision array of features and samples representing the year of data.

% Which feature importance algos were used?
Impurity-based feature importance experiments were conducted using the Mean Decrease in Impurity method, which measures the mean and standard deviation of accumulation of the impurity decrease within each decision tree, using an MSE loss function \citep{tibshirani_elements_2001}.
...
\end{equation}
where $N_t$ is the number of samples passed through node $t$, $y_i$ is the target values, and $\bar{y}_t$ is the mean target value at the node.
...
@Sophie You've not actually said anything here about model design or eluded to what the design criteria are. I think if you have a section heading the first paragraph should connect squarely with that heading. This isn't the case in say literature but in scientific writing I think it is. 

Both of the photolysis codes in UKCA, Fast-JX and the stratospheric lookup-table, were emulated by the random forest to produce a single photolysis scheme for all altitudes.

Arranging the input data into a two-dimensional matrix of samples and features allowed the random forest to be transferable to different spatial dimensions and timestep frequencies. A grid box, a column, the entire global grid, or any shaped sample of the grid can be passed to the random forest without any added processing required. The UM calls photolysis on Message Passing Interface (MPI) domains which are typically a subset of the Earth grid, split by latitude and longitude, with full columns retained. The random forest's data structure makes it transferable to these domains as well as any other grid shape, such as a single-column model. This is preferable to physics-based photolysis models which require specific spatial dimensions and timestep frequencies.

% Single.
A single random forest can predict all reactions with computational efficiency; decision trees are traversed based on input feature values and the leaf nodes hold one-dimensional arrays of prediction values, so a random forest of the same structural size takes practically the same amount of time to predict many targets as to predict one target. 
...
\begin{figure}[htpb]
    \centering 
    \includegraphics[scale=0.9]{images/Forest n trees MSE.svg}
...
\begin{figure}[htpb]
    \centering 
    \includegraphics[scale=0.95]{images/n trees R2.svg}
...
% Forest size.
Random forests were constructed and tested with 1 to 50 trees. Figures \ref{n trees MSE} and \ref{n trees R2} exhibit MSE and \RR\ scores of predictions to targets from random forests containing different numbers of decision trees. For most J-values, the random forest performed best with 5 to 20 trees, with 20 trees identified as the ideal for a multi-output model predicting all J-values.
% Depth > size.
Tests on the random forest configuration indicated that model performance was more strongly influenced by tree depth than by the total number of trees. For some reactions, such as \JHHOO, a single deep decision tree achieved predictive skill comparable to that of a random forest comprising 20 shallower trees. For these reactions, increasing the depth of a single tree yielded further marginal improvements, while increasing the number of trees provided little additional benefit. These results suggest that, for some of the reactions examined, the ensemble averaging of the random forest offered little improvement on individual decision trees, indicating that the underlying input–output relationships were relatively simple and well captured by a single, sufficiently expressive model.

% Best design.
A random forest of few, deep trees was identified as the most effective design for emulating UKCA J-values. The final design chosen included 20 trees of 199,999 nodes each, trained on a one-year dataset of 500,000 uniform randomly sampled data points per day and further randomly sampled at 10\% to 20\% per tree.   
...
\begin{figure}[htpb]
    \centering 
    \includegraphics[scale=0.65]{images/ISON trop med time.png}
...
For some reactions, the model performed better in the stratosphere than in the troposphere. The affected reactions were \JOCS, \JISON, \JHHO, \JNNO, \JNO, \JmeCHOCHHHH\ and \JOOP. All of these reactions occurred mostly in the stratosphere and had particularly sharp increases in J-value by altitude. The random forest typically made small absolute errors in their predictions, although the errors were significant in the troposphere, where the target values were zero or close to zero, but the random forest predicted a small positive value, leading to an average over-prediction of these J-values in the troposphere, as illustrated in Figure \ref{ISON days med}.

% Masking method.
The specific problem J-values were masked to zero at low altitudes to mitigate the overprediction of small J-values at low altitudes.
Vertical model levels were chosen as a criterion for applying a mask because their values were easy and efficient to check online within the photolysis routines in UKCA. The method applied was a hard cut-off, with all values below the masking threshold set to zero and all those above it left unaltered. 
...
\begin{figure}[htpb]
    ...
    \centering
    \includegraphics[scale=0.8]{images/O2 masked preds lenient no title.png}
    ...
    \begin{subfigure}{\linewidth}
    \centering
    \includegraphics[scale=0.8]{images/O2 masked preds strict no title.png}
    \caption{\textit{\JOOP\ masked too high.}}
    \label{mask too high}
    \end{subfigure}

    \caption{\textit{Global daily mean \JOOP\ predictions and targets below model level 70 (approximately 35 km above sea-level) with too low and too high mask thresholds.}}
...
\begin{figure}[htpb]
    ...
    \begin{subfigure}{\linewidth}
    \centering
    \includegraphics[scale=0.65]{images/HCHOr.png}
    ...
    \begin{subfigure}{\linewidth}
    \centering
    \includegraphics[scale=0.65]{images/propanal.png}
...
% Performance summary.
The random forest performed well overall and was capable of emulating both Fast-JX and the 2D lookup table photolysis scheme offline. Figure \ref{corr V good} shows examples of some of the best-predicted J-values, \JHCHOr\ and \Jpropanal. Most other J-values displayed similarly satisfactory correlations. 

25 out of 26 (96\%) of the emulated reactions achieved \RR\ scores above 0.9. The exception was 0.79, for \JHHO. 23 (92\%) of the reactions were emulated with overall global \RR\ scores above 0.95. 13 (50\%) of the reactions were emulated with overall global \RR\ scores above 0.99.
The overall \RR\ score of all predictions to targets from a global, year-long test dataset was 0.97 and the MSE was $3.85 \times 10^{-7}$. 
...
\begin{sidewaysfigure}[htpb]
    \centering 
    \includegraphics[width=\textheight]{images/Cl_mass mixing ratio_1y.png}
...
% Why Cl is useful.
The chlorine radical is representative of halogen radical chemistry, which is sensitive to changes in UV and ozone column \citep{finlayson-pitts_chemistry_2000}. Key reactions include
...
\begin{sidewaysfigure}[htpb]
    \centering 
    \includegraphics[width=\textheight]{images/OH_mass mixing ratio 1y.png}
...
\begin{sidewaysfigure}[htpb]
    \centering 
    \includegraphics[width=\textheight]{images/H₂O_mass mixing ratio 1y.png}
    \caption{\textit{Global daily mean water mass mixing ratio from one-year simulations by UKCA with the original and the random forest photolysis schemes.}}
    \label{H2O MMR}
\end{sidewaysfigure}
...
$O(^1D) + H_2O \rightarrow 2OH$ \citep{jacob_introduction_1999}.
% Water results.
Figure \ref{H2O MMR} exhibits mean water mass mixing ratios over one-year simulations. 
UKCA with emulated photolysis produced similar water mass mixing ratios to those in UKCA with the original photolysis scheme, suggesting that the masking out of low-altitude water photolysis did not harm the mean water content of the atmosphere. This was supported by viewing water over different timescales and in portions of the atmosphere.  

% N2O results.
\begin{sidewaysfigure}[htpb]
    \centering 
    \includegraphics[width=\textheight]{images/N₂O_mass mixing ratio 1y.png}
    \caption{\textit{Daily mean $N_2O$ mass mixing ratio from a one-year simulations by UKCA with the original and the random forest photolysis schemes.}}
    \label{N2O MMR}
\end{sidewaysfigure}

% Why N2O is useful.
N$_2$O is useful for viewing effects in the upper atmosphere, as it undergoes photolysis mainly in the stratosphere \citep{world_meteorological_organization_scientific_1998}. Relevant reactions include
$N_2O + h\nu \rightarrow N_2 + O(^1D)$ and
$N_2O + O(^1D) \rightarrow 2NO$.
% N2O results.
Figure \ref{N2O MMR} illustrates mean N$_2$O mass mixing ratios over one-year simulations. The daily mean from UKCA with the random forest diverged from the target values over the first model year, before stabilising with a low bias towards the end of the year.
UKCA with emulated photolysis reproduced the same overall trend as the original version of UKCA, both annually and over a 30-year test period, although in many months, the global mean was slightly lower than the reference value. 

% NO results.
\begin{sidewaysfigure}[htpb]
    \centering 
    \includegraphics[width=\textheight]{images/NO_mass mixing ratio 1y.png}
...
% Why NO is useful.
NO is a product of NO$_x$ photolysis and affects ozone production \citep{seinfeld_atmospheric_2016}. Some key reactions are
...
\begin{sidewaysfigure}[htpb]
    \centering 
    \includegraphics[width=\textheight]{images/peroxyacetyl nitrate mass mixing ratio 1y.png}
    \caption{\textit{Daily mean global PAN mass mixing ratio from UKCA with the original and random forest photolysis schemes, over a one-year simulation.}}
    \label{PAN}
\end{sidewaysfigure}

% Why PAN is useful.
Peroxyacetyl nitrate (PAN) is a reservoir for NO$_x$ in the troposphere, affecting ozone and OH in the atmosphere \citep{fischer_atmospheric_2014}. Compared to the other species considered, PAN is more sensitive to temperature, one of the input features for the random forest. An important reaction is
...
% PAN results.
Figure \ref{PAN} presents one-year daily mean PAN mass mixing ratios from UKCA running online in the UM with both photolysis schemes. 
Daily mean PAN was underestimated by UKCA containing the random forest, particularly at troughs from December to April, and less so at peaks from May to July. 
In a 30-year run, the random forest version slightly underestimated the amplitude of the peaks and troughs of monthly averages, but followed the trend well and remained stable throughout the simulation. 

% SW heat results.
\begin{sidewaysfigure}[htpb]
    \centering 
    \includegraphics[width=\textheight]{images/Shortwave heating rates_1y.png}
...
Shortwave heating rates are used to measure the radiative impact of photolysis changes and are affected by ozone absorption and UV flux changes \citep{hartmann_global_2016}. 
% SW heat results.
...
Ozone is particularly important for both photochemistry and radiation and is sensitive to NO$_x$ and HO$_x$ \citep{jacob_introduction_1999}.
Total column ozone affects UV penetration and feeds back to photolysis rates, making it a useful parameter for checking consistency between radiation and photolysis \citep{world_meteorological_organization_scientific_1998}. O$_x$ production measures net ozone production and is an indicator of biases in chemical production rates \citep{finlayson-pitts_chemistry_2000}. Some relevant reactions are 
...
\begin{sidewaysfigure}[htpb]
    \centering 
    \includegraphics[width=\textheight]{images/O₃ mass mixing ratio 1y.png}
    \caption{\textit{Global daily mean ozone mass mixing ratio from one-year simulations by UKCA with the original and the random forest photolysis schemes.}}
    \label{O3 MMR}
\end{sidewaysfigure}

% O3 MMR results.
Figure \ref{O3 MMR} shows mean ozone mass mixing ratios over a year. 
UKCA with emulated photolysis generated similar trends of ozone over time. It moderately underestimated global mean ozone throughout a 30-year simulation, although the error remained consistent after the first nine months of the model run. This was due to ozone being overestimated in the stratosphere but not at all in the troposphere, potentially implying that the model could benefit from the masking threshold of one or more of the J-values being increased in altitude, or that a smoother transition may be required from zero to the predicted J-values across the masking threshold.

% Ox results.
\begin{sidewaysfigure}[htpb]
    \centering 
    \includegraphics[width=\textheight]{images/Oₓ production 1y.png}
    \caption{\textit{Global daily mean $O_x$ production from one-year simulations by UKCA with the original and the random forest photolysis schemes.}}
    \label{Ox prod online}
\end{sidewaysfigure}

% Ox results.
Figure \ref{Ox prod online} presents mean O$_x$ production over one-year simulations. 
UKCA with emulated photolysis displayed the same patterns over time as UKCA with the original photolysis scheme, but underestimated O$_x$ production on average throughout the model run, particularly for higher average J-values from May to July. Masking some J-values to zero at low altitudes had been shown to affect O$_x$ production, as presented in Figure \ref{Ox prod lvl mask}, and the low mean bias suggests that this could be further improved.
...
\begin{figure}
    \centering 
    \includegraphics[scale=0.9]{images/1-month timings.png}
...
1. Use the chosen method to accurately emulate UKCA photolysis rate coefficients, with a global coefficient of determination of at least 0.9 for each emulated reaction.
...
25 out of 26 (96\%) of the emulated reactions achieved this aim in both the offline and online models. The only reaction which was not emulated with the desired predictive performance was \JHHO. The small J-values of water photolysis and certain reactions in lower-altitude portions of the atmosphere were particularly difficult to emulate using the chosen method and may be improved by a different, targeted ML approach.\\

2. Use the chosen model to speed up the run time of UKCA within the UM, reducing the run time of the photolysis portion of UKCA by at least 20\%.

This goal was achieved and the target was surpassed, with the implemented emulator reducing the run time of the photolysis portion of UKCA by 85.7\% on all chemistry timesteps except the first, although the emulation was only faster than the original photolysis scheme after 3 simulated days due to its long initialisation time. Converting the emulator from Python to Fortran via a NetCDF file was successful, allowing ML models to be trained in Python and integrated into UKCA for online inference. The proof of concept implementation should not currently be used to fully replace the other photolysis schemes in UKCA because not all of UKCA's J-values have been added to its output set, but this work has demonstrated that it runs successfully in UKCA, predicts most J-values adequately and can be extended to output all J-values and replace the other photolysis schemes.\\ 
...
\section{Emulated UKCA Photolysis Reactions}\label{rxns}    
%% Appendix A

\begin{center}
\begin{tabular}{ |c|c|c| }
\hline
Formula & Reagent & Products \\
\JNO & $NO$ & $N + O(^3P)$ \\
\JNOO & $NO_2$ & $NO + O(^3P)$ \\
\JNOOO & $NO_3$ & $NO + O_2$ \\
\JNOx & $NO, NO_2\ or\ NO_3$ & $As\ above$ \\
\JNNO & $N_2O$ & $N_2 + O(^1D)$ \\
\JOO & $O_2$ & $2O(^3P)$ \\
\JOOO & $O_3$ & $O_2 + O(^1D)$ \\
\JHCHOr & $HCHO$ & $2HO_2 + CO$ \\
\JHCHOm & $HCHO$ & $H_2 + CO$ \\
\JClClOO & $Cl_2O_2$ & $2Cl + O_2$ \\
\JOCS & $OCS$ & $CO + SO_2$ \\
\JSOOO & $SO_3$ & $SO_2 + O(^3P)$ \\
\JISON & $Isoprene\ nitrates$ & $NO_2 + HCHO + HO_2 + CH_2C(CH_3)CHO$ \\
\JmeOOH & $CH_3OOH$ & $HO_2 + HCHO + OH$ \\
\JmeONOO & $CH_3ONO_2$ & $HO_2 + HCHO + NO_2$ \\
\JmeCHOmeOO & $CH_3CHO$ & $CH_3O_2 + HO_2 + CO$ \\
\JmeCHOCHHHH & $CH_3CHO$ & $CH_4 + CO$ \\
\JmeCOCHO & $CH_3COCHO$ & $CH_3CO_3 + CO + HO_2$ \\
\JCHHHH & $CH_4$ & $CH_3O_2 + H$ \\
\Jpropanal & $CH_3CH_2CHO$ & $CH_3CH_2O_2 + HO_2 + CO$ \\
\Jacetone & $CH_3COCH_3$ & $CH_3CO_3 + CH_3O_2$ \\
\JHOBr & $HOBr$ & $OH + Br$ \\
\JHOCl & $HOCl$ & $OH + Cl$ \\
\JHNOOO & $HNO_3$ & $OH + NO_2$ \\
\JHNOOOO & $HNO_4$ & $OH + NO_3$ \\
\JHHO & $H_2O$ & $OH + H$ \\
\JHHOO & $H_2O_2$ & $2OH$ \\
\JHHSOOOO & $H_2SO_4$ & $SO_3 + OH + H$ \\
\JMACR & $CH_2C(CH_3)CHO$ & $CH_3CO_3 + HCHO + CO + HO_2$ \\
\hline
\end{tabular}
\end{center}
...
% R2 
The main accuracy metric chosen for model assessment was the coefficient of determination, also known as the \RR\ score, which measures the proportion of variance in the dependent variable that is predictable from the independent variables \citep{draper_applied_1998}. 
...
Using the \RR\ score as an accuracy metric sometimes makes predictions appear poor for the smallest photolysis rates. In these cases, it is useful to use additional metrics to gain a better understanding of the accuracy and fit.
...
\authorcontribution{ATA and LA envisioned the project. ST curated the data, developed the ML model, performed the analysis and wrote this paper under the guidance of ATA and LA.} %% this section is mandatory
